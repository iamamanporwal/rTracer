...
describe('applyCloudKey — dark (cloud is grey on a white background)', () => {
  ...
  it('keeps (and brightens) the cloud shading rather than forcing white', () => {
    ...
    applyCloudKey(data, 'dark');
    // Original ordering preserved, each channel nudged up by 36, clamped to 255.
    expect(data[0]).toBe(186);
    expect(data[1]).toBe(191);
    expect(data[2]).toBe(201);
  });
});

describe('featherStamp — radial fade + underside shade', () => {
  ...
  it('feathers radially: rim → 0, centre stays opaque, no rectangle', () => {
    const w = 64;
    const h = 64;
    const d = solid(w, h);
    featherStamp(d, w, h);
    const at = (x: number, y: number, c: number): number => d[(y * w + x) * 4 + c]!;
    // Corners and edge midpoints feather out — no hard rectangular edge survives.
    expect(at(0, 0, 3)).toBe(0);
    expect(at(w - 1, h - 1, 3)).toBe(0);
    expect(at(Math.floor(w / 2), 0, 3)).toBe(0);
    expect(at(0, Math.floor(h / 2), 3)).toBe(0);
    // Centre stays fully opaque.
    expect(at(Math.floor(w / 2), Math.floor(h / 2), 3)).toBe(255);
  });

  it('darkens the underside (lower half) so the billboard reads as volume', () => {
    const w = 8;
    const h = 8;
    const d = solid(w, h);
    featherStamp(d, w, h);
    // Centre column, comparing an upper row to a lower one (RGB shade, not alpha).
    const cx = Math.floor(w / 2);
    const topRGB = d[(2 * w + cx) * 4]!;
    const botRGB = d[((h - 3) * w + cx) * 4]!;
    expect(botRGB).toBeLessThan(topRGB);
  });
});
